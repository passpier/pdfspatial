...
Status reflects the Stage 3 regression corpus under [`fixtures/`](../fixtures) — run `cargo run --example stage3_scoreboard --features stage3` for the live scoreboard (currently 11/26 cases passing); see [`fixtures/README.md`](../fixtures/README.md) for per-case detail.
...
- [ ] **Cross-page table/paragraph continuation** — tables or paragraphs split across a page boundary not stitched back together *(1 case, `ordering`; 0/1 passing)*
