...
| graphics::detect_table_regions (Stage 1b) only detects tables from ruling lines (Page::graphics), which a borderless table has none of by definition -- there is nothing for it to key off here, same as before Stage 1b existed. | Borderless / whitespace-delimited tables | A text-alignment heuristic in layout.rs: column-consistent x-starts across several consecutive lines, independent of graphics.rs's ruling-line signal. |
| Unlike Table/Picture, a formula has no reliable non-text page-object signal (no ruling lines, no XObject) to key a geometric heuristic off -- it's ordinary glyphs in an unusual spatial arrangement (stacked fraction bars, inline symbol clusters), which is genuinely a layout/vision-model problem, not an unused-PDFium-API problem. | Nested mathematical formulas | Stage 4b vision detector -- the one region class in this registry that still needs it. |
