...
Legend: `[x]` fixed · `[~]` partial · `[ ]` open. Status reflects the Stage 3
regression corpus under [`fixtures/`](../fixtures) — run
`cargo test --features stage3 -- --ignored` for the live scoreboard (currently
11/26 cases passing); see [`fixtures/README.md`](../fixtures/README.md) for
per-case detail.

- [x] **Multi-column layout / gutter detection** — text lines merged across column boundaries, or column order inverted (right column read before left) *(3 corpus cases, `ordering`; 3/3 passing — `assemble::assemble_reading_order`'s XY-cut now qualifies a vertical gutter against the page's own median character size (`MIN_GUTTER_EMS`), not just a fixed page-width fraction (`MIN_CUT_FRACTION`), so narrow (1 em) column gutters recover column-major order the same way wide ones already did)*
- [~] **Footnotes** — footnote text merged into body `Text` region instead of classified as `Footnote`; footnote markers (superscript numerals) not linked to their note *(2 cases, `classification`; 2/2 classified via `layout::classify_block`'s footnote rule — but `serialize::render_block` has no `Footnote` arm yet, so marker-to-note linking/rendering is still open)*
- [x] **Page headers/footers repeated across pages** — header/footer text leaking into body text stream; running headers misclassified as `Section-header` *(3 cases, `classification`; 3/3 passing — `layout::band_of` shape test + `repeated_running_bands` cross-page pass, suppressed in `serialize::render_block`)*
- [ ] **Nested/multi-line table cells** — cells spanning multiple visual lines merged incorrectly with adjacent rows; rowspan/colspan not detected *(1 PDF-backed case, `classification`; 0/1 — blocked, no code path emits `RegionClass::Table`; also flagged `requires_extraction_fix`)*
- [ ] **Merged table cells (rowspan/colspan)** — TEDS-Struct penalizes tree-topology mismatches here most heavily *(2 cases, `classification`; 0/2 — blocked, no code path emits `RegionClass::Table`)*
- [ ] **Borderless / whitespace-delimited tables** — no ruling lines for the layout model to key off; frequently misclassified as plain `Text` *(2 cases, `classification`; 0/2 — blocked, no code path emits `RegionClass::Table`)*
- [ ] **Nested mathematical formulas** — inline formulas embedded mid-sentence not separated from surrounding text; multi-line/stacked formulas (fractions, summations, matrices) fragmented into multiple bounding boxes *(2 cases, `classification`; 0/2 — blocked, no code path emits `RegionClass::Formula`)*
- [x] **Superscript/subscript handling** — footnote markers, exponents, and chemical/math subscripts causing baseline-clustering errors in Stage 1's line grouping *(1 PDF-backed case, `geometric`; 1/1 passing — `extract::group_words`'s `is_script_continuation` check)*
- [ ] **Rotated or vertical text** — sidebar labels, rotated table headers, CJK vertical text (relevant given Traditional Chinese/Japanese document support) *(1 PDF-backed case, `geometric`; 0/1 — `extract_char` doesn't read PDFium's rotation matrix; flagged `requires_extraction_fix`)*
- [ ] **Figure/caption association** — captions not correctly linked to their parent `Picture`/`Table` region, or associated with the wrong figure when multiple figures share a page *(2 cases, `classification`; 0/2 — `layout::is_caption` covers the caption half, but no code path emits `RegionClass::Picture`)*
- [~] **List-item nesting** — multi-level bullet/numbered lists collapsed into flat `List-item` blocks, losing hierarchy *(2 cases, `ordering`; 2/2 reading-order passing after the narrow-gutter fix shared with multi-column above — each side-by-side outline is now read fully before the next; hierarchy/depth itself is still not modeled, so nested lists still flatten into one flat sequence)*
- [ ] **Cross-page table/paragraph continuation** — tables or paragraphs split across a page boundary not stitched back together *(1 PDF-backed case, `ordering`; 0/1 — `assemble_reading_order` operates per-page by construction; flagged `requires_extraction_fix`)*
- [ ] **Section headers vs. bold body text** — `Section-header` misclassified as `Title` or emphasized `Text` when font-weight is the only visual cue *(2 cases, `classification`; 0/2 — blocked upstream of the classifier: the corpus line schema carries `font_size` only, no font name/weight signal)*
- [ ] **Embedded/CID-keyed fonts and ligatures** — pdfium character extraction dropping or garbling glyphs from non-standard font encodings *(1 PDF-backed case, `geometric`; 0/1 — flagged `requires_extraction_fix`)*
- [ ] **Overlapping/z-ordered text objects** — watermarks, stamps, or redaction boxes overlapping real text and corrupting bbox clustering *(1 PDF-backed case, `geometric`; 0/1 — flagged `requires_extraction_fix`)*

#### Status & next steps

| Blocker | Pitfalls | Next step |
|---|---|---|
| No code path emits `Table`/`Formula`/`Picture` | Nested/multi-line table cells, merged table cells, borderless tables, nested formulas, figure/caption | Stage 4b vision detector — the documented `unimplemented!()` stub |
| Stage 1 extraction gaps (`requires_extraction_fix`) | Rotated/vertical text, embedded/CID-keyed fonts, overlapping/z-ordered text | Read PDFium's rotation matrix / encoding table / z-order in `extract.rs` |
| `assemble_reading_order` is single-page | Cross-page continuation | Cross-page stitching (Stage 4a, third bullet) |
| Corpus schema carries no font-weight signal | Section headers vs. bold | Extend `extract`/the corpus JSON schema with font name/weight *before* touching the classifier |
| Footnote rendering not wired up | Footnotes (partial) | Add a `RegionClass::Footnote` arm to `serialize::render_block`; link markers to their note |
...
**Exit criterion:** Every pitfall category above has a labeled regression corpus and a root-cause tag; failure counts are ranked to define the Stage 4 priority order. **Met** — all 15 pitfalls are seeded in the corpus with a root-cause tag and a live pass/fail scoreboard (11/26 cases passing today, ranked into blocker clusters above); the one open gap is volume — each category has 1–3 cases, short of the ≥ 20-samples-per-category target, which needs a real DocLayNet mining pass to close.
...
| 3. Error Analysis | Failure count/category, root-cause split | Full pitfall-checklist coverage — **met**: 15/15 pitfalls seeded, root-cause-tagged, scoreboard-tracked (11/26 cases passing); ≥20-samples/category volume still outstanding |
