...
- [x] **Section headers vs. bold body text** — `Section-header` misclassified as `Title` or emphasized `Text` when font-weight is the only visual cue *(2 cases, `classification`; 2/2 passing — layout::char_is_bold now prefers Char::font_weight over the font-*name* heuristic (layout::is_bold_font_name) whenever the font program declares a plausible weight -- closing the gap this pitfall was blocked on. One wrinkle found while wiring this up against the real opendataloader-bench corpus (not the synthetic corpus, which can't reproduce it): PDFium has been observed reporting Char::font_weight: Some(0) for a font it can't actually resolve a weight for (a real Lato-Bold character reporting weight 0, not 700 and not None), which would silently override is_bold_font_name's correct answer if trusted literally. char_is_bold's MIN_PLAUSIBLE_FONT_WEIGHT floor (100) treats an implausibly low declared weight as unreliable and falls back to the name heuristic instead -- see layout.rs's zero_font_weight_falls_back_to_font_name test.)*
